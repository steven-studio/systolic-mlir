# -----------------------------------------------------------------------------
# bw_build.tcl -- build and program the DDR3 bandwidth probe.
#
#   vivado -mode batch -source bw_build.tcl -tclargs build
#   vivado -mode batch -source bw_build.tcl -tclargs program
#   vivado -mode batch -source bw_build.tcl -tclargs all      (both)
#
...
set PROJ_DIR  $::env(HOME)/work/vivado/ddr3_bw
set PROJ_NAME ddr3_bw
set TOP       ddr3_bw_top
set JOBS      8

set SCRIPT_DIR [file dirname [file normalize [info script]]]
set XCI        $SCRIPT_DIR/ip/ddr3/mig_7series_0.xci
set SRC        [list $SCRIPT_DIR/ddr3_bw_top.sv $SCRIPT_DIR/ddr3_bw_probe_axi.sv $SCRIPT_DIR/uart_tx.sv]
set XDC        $SCRIPT_DIR/nexys_video_bw.xdc
...
proc build {part proj_dir proj_name top xci src xdc jobs board_repo board_part} {
    ...
    if {[catch {
        ...
        if {$had_prev} {
            ...
            puts "    vivado -mode batch -source bw_build.tcl -tclargs read"
...
proc build_body {part proj_dir proj_name top xci src xdc jobs board_repo board_part} {
    ...
    #     '.../ddr3_bw.gen/sources_1/ip/mig_7series_0': no such file or directory
...
proc program {proj_dir proj_name} {
    ...
    puts "    led\[1\]  probe issuing reads    (brief, ~0.7 ms -- you may miss it)"
    puts "    led\[2\]  probe finished         stays lit while reporting"
    puts "    led\[3\]  our 200 MHz MMCM locked"
    puts "    led\[4\]  heartbeat, 100 MHz"
    puts "    led\[5\]  heartbeat, ui_clk"
    puts ""
    puts "  Then open a terminal at 115200 8N1.  The report repeats every ~1 s:"
    puts "     BEATS=00010000 CYC=0001C4B7 SINK=8B3C10F7"
    puts ""
    puts "     beta_ceiling = 4 * BEATS / CYC   words per 100 MHz array cycle"
    puts "     compare against 11.2 (fold-average demand at s=8, K=256, H=95)"
    puts ""
    puts "  led\[0\] and led\[2\] lit, no UART -> CLK_HZ does not match ui_clk"
    puts "  BEATS=0 with CYC large            -> AR accepted, no data returned"
    puts "  SINK=00000000                     -> all-zero data; DRAM was never"
    puts "                                       written, or the bus is dead"
    close_hw_manager
    close_project
}

# -----------------------------------------------------------------------------
proc read_vio {proj_dir proj_name} {
    open_project $proj_dir/$proj_name.xpr
    open_hw_manager
    connect_hw_server
    set names {}
    ...
    set beats [pval $all_probes HEX    dbg_beats probe_in0]
    set cyc   [pval $all_probes HEX    dbg_cyc   probe_in1]
    set sink  [pval $all_probes HEX    dbg_sink  probe_in2]
    set calib [pval $all_probes BINARY init_calib_complete]
    set run   [pval $all_probes BINARY probe_running]
    set rep   [pval $all_probes BINARY probe_reported]
    ...
    puts "  probe_running       = $run     (reads in flight)"
    puts "  probe_reported      = $rep     (window closed, numbers latched)"

    if {$beats ne "" && $cyc ne ""} {
        # NOT [expr {0x$beats}]: Tcl 9 (Vivado 2026.1) refuses a variable
        # substituted into a numeric literal -- "invalid bareword 0x".
        set b 0 ; scan $beats %x b
        set c 0 ; scan $cyc   %x c
        puts ""
        puts "  BEATS  = $b"
        puts "  CYC    = $c"
        if {$c > 0} {
            puts [format "  beta_ceiling  = 4 * %d / %d = %.3f words per 100 MHz array cycle" \
                  $b $c [expr {4.0 * $b / $c}]]
            puts [format "  efficiency vs the 4.00 config ceiling: %.1f%%" \
                  [expr {100.0 * $b / $c}]]
            puts [format "  sustained     = %.3f GB/s  (%d bytes in %.1f us)" \
                  [expr {16.0 * $b / ($c * 1.0e-8) / 1.0e9}] \
                  [expr {16 * $b}] [expr {$c / 100.0}]]
            puts "  fold-average demand for comparison: 11.2 words/cycle"
            puts [format "  the operand path is short by %.2fx" \
                  [expr {11.2 * $c / (4.0 * $b)}]]
        }
        if {$b != 65536} {
            puts ""
            puts "  NOTE: expected 65536 beats (4096 bursts x 16).  Fewer means the"
            puts "        run timed out before every burst returned."
        }
    }
    close_hw_manager
    close_project
...
switch -- $MODE {
    ...
    read    { read_vio $PROJ_DIR $PROJ_NAME }
