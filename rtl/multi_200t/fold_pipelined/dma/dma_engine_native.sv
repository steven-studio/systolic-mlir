// -----------------------------------------------------------------------------
// dma_engine_native.sv -- ARCHIVED.  The original descriptor-driven read engine,
// written against the MIG 7-series NATIVE user interface.
//
// WHY THIS IS NOT THE ONE IN USE
//   The MIG instance in this design is generated from nexys_video_mig_axi128.prj,
//   which configures an AXI4 slave interface (C0_S_AXI_DATA_WIDTH = 128) -- there
//   is no app_en / app_rdy / app_rd_data_valid to talk to.  This file is kept
//   verbatim (only the module name changed) because:
//
//     * its descriptor front end, credit accounting and statistics are carried
//       over unchanged into dma_engine.sv, and having the original beside the
//       port makes the diff reviewable;
//     * if MIG is ever regenerated with the native UI, this is still correct;
//     * its bench (tb/tb_dma_engine.sv, 4/4 under Icarus and xsim, mutation
//       tested) documents behaviour the AXI version must reproduce.
//
//   Nothing instantiates it.  See dma_engine.sv for the live engine.
//
