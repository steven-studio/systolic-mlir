...
need build_kmax.tcl        "systolic_array_4x4"      "4x4 包裝進 read_verilog 清單"
need systolic_pe.sv        "u_fp_add_reduce"         "歸約用的第二顆加法器"
need systolic_pe.sv        "acc_bank \[0:1\]"         "乒乓的兩組 bank"
need systolic_tx_source.sv "4 \* N \* N"              "TX 已砍半(ctx 移除)"
...
need test_uart_kmax.py     "2 \* (n - 1)"             "幾何化 cycle 模型"
need test_uart_kmax.py     "h_meas"                   "cycle 模型改為量測 H(不再檢查 105)"
need test_uart_kmax.py     "expected_result"          "ctx 移除:回應只剩一片"
need test_uart_kmax.py     "tx_bytes = 4 \* n \* n"     "回應已砍半"
need systolic_uart_top.sv         "parameter int N = 8"     "N generic"
