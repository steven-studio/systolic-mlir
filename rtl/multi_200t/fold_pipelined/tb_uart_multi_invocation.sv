...
module tb_uart_multi_invocation;
    ...
    // 陣列邊長(隨 top 的 N generic)。wire format 的 lane 數與
    // 期望結果矩陣都隨之縮放;8-deep k window 是協定常數不變。
    parameter int N     = 8;
    ...
    task automatic send_request(input int k_dim, input int base_k);
        ...
        begin
            ...
            for (w = 0; w < K_MAX / 8; w++) begin
                // A window: [row][local k] -- N 個 lane、8 個 k offset
                for (r = 0; r < N; r++)
                    for (c = 0; c < 8; c++) begin
                        lk = w * 8 + c;
                        send_fp32(lk < k_dim ? a_val(r, base_k + lk) : POISON);
                    end
                // B window: [local k][col] -- 8 個 k offset、N 個 lane
                for (r = 0; r < 8; r++)
                    for (c = 0; c < N; c++) begin
                        lk = w * 8 + r;
    ...
    // -------------------------------------------------------------------
    shortreal exp0 [0:N-1][0:N-1];
    shortreal exp1 [0:N-1][0:N-1];
    shortreal acc  [0:N-1][0:N-1];
    shortreal ref_c[0:N-1][0:N-1];   // NOT "ref": SystemVerilog reserved word
    ...
    initial begin
        ...
        $display("");
        $display("==============================================");
        $display(" K_MAX = %0d   K = %0d   N = %0d  -> %0d invocation(s)",
                 K_MAX, K, N, N_INV);
        $display("==============================================");
    ...
    initial begin
        ...
        /* TX 重構後 tx_state 已不存在,改讀 tx_source 的狀態。 */
        $display("   state=%0d feed_t=%0d tx_src=%0d",
                 dut.state, dut.feed_t, dut.u_tx_source.src);
        $display("   PE 完成數 = %0d / %0d   all_results_valid=%0b out_state=%0d",
                 pes_done(), N * N, dut.u_array.all_results_valid,
