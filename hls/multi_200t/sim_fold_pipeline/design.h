...
// width at 32 bits, and matmul_iface_rk.v and the host both assume that.
...
// Top-level kernel
// ---------------------------------------------------------------------
// Renamed 2026-08-06: matmul_4x4x4 -> matmul_8x8x8. Verified as a pure
// refactor -- csynth instruction counts, II, Depth and Fmax all unchanged,
// and place/route produced an identical netlist (same WNS, same router
// overlap sequence). Hardware regression 6/6 bit-exact.
//
// The name is still only two-thirds honest: R and C are macros that happen
// to default to 8, but the trailing 8 is wrong -- K is a runtime argument
// in [1, K_MAX] and K_MAX is 64. Read it as "8x8 array", not "8x8x8 GEMM".
//
// Renaming again is not free. The top-level function name becomes the
// generated Verilog module name, so a rename means: this prototype, the
// definition in design.cpp, syn.top and syn.directive.pipeline in
// hls_rk.cfg, and the instantiation emitted by rtl/gen_matmul_iface_new.py
// -- then regenerate matmul_iface_rk.v, point vivado/build_project.tcl at
// the new work dir, and rebuild. Budget ~15 minutes.
//
...
void matmul_8x8x8(data_t A[R][K_MAX], data_t B[K_MAX][C], data_t Cinout[R][C],
                  int K);
