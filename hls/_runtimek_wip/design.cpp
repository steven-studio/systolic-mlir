...
// per-invocation: measured cost is K + R + C - 2 + 6, not K + R + C - 2.
// At K=64 on 8x8 that is 84 against an ideal 78 -- 7.7%. See above for why
// closing that gap is deliberately out of scope here.
...
// Name kept as-is despite being stale (K is runtime, R/C are macros). The
// top-level function name becomes the generated module name, so renaming it
// would force an edit to matmul_iface.v for no functional gain. Rename it
// on the next pass that touches the RTL anyway.
void matmul_4x4x4(data_t A[R][K_MAX], data_t B[K_MAX][C],
